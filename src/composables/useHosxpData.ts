...
import type { HosxpDrugItem, HosxpDrugMonthlyData, HosxpDrugSummary } from '../stores/dashboard';
...
export function useHosxpData() {
  ...
  async function fetchDrugMonthly(
    year: number,
    icode: string,
  ): Promise<HosxpDrugMonthlyData | null> {
    store.loadingChart = true;
    try {
      const result = await invoke<HosxpDrugMonthlyData[]>('hosxp_get_drug_monthly_qty', {
        year,
        icode,
      });
