...
export function useInvsData() {
  ...
  async function fetchTopDrugs(year: number, limit = 10): Promise<InvsDrugValueSummary[]> {
    try {
      const drugs = await invoke<InvsDrugValueSummary[]>('invs_get_top_drugs_by_value', {
        year,
        limit,
      });
  ...
  async function fetchDrugMonthlyValue(
    year: number,
    workingCode: string,
  ): Promise<InvsDrugMonthlyValue | null> {
    store.loadingChart = true;
    try {
      const data = await invoke<InvsDrugMonthlyValue>('invs_get_drug_monthly_value', {
        year,
        workingCode,
      });
