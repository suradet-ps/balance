...
import { FISCAL_MONTHS_SHORT, formatBaht, formatQty, THAI_MONTHS_SHORT } from '../utils/dateUtils';
...
function getCode(d: ChartData): string {
  return d.icode ?? d.working_code ?? '';
}
function getName(d: ChartData): string {
  return d.drug_name ?? d.name ?? '—';
}
function getTotal(d: ChartData): number {
